...
import { O2Rum, InternalLog, SdkVerbosity } from '@openobserve/mobile-react-native';
 ...
 */
export class O2RumReactNativeNavigationTracking {
    ...
    static startTracking(trackingOptions?: NavigationTrackingOptions): void {
        ...
        if (O2RumReactNativeNavigationTracking.isTracking) {
            return;
        }
        ...
        O2RumReactNativeNavigationTracking.eventSubscription = Navigation.events().registerComponentDidAppearListener(
            (event: ComponentDidAppearEvent) => {
                const predicate =
                    O2RumReactNativeNavigationTracking.viewNamePredicate;
                const screenName = predicate(event, event.componentName);
                const shouldTrack = O2RumReactNativeNavigationTracking.viewTrackingPredicate(
                    event
                );
                if (screenName !== null && shouldTrack) {
                    const passProps = O2RumReactNativeNavigationTracking.paramsTrackingPredicate(
                        event
                    );

                    if (passProps) {
                        O2Rum.startView(event.componentId, screenName, {
                            passProps
                        });
                    } else {
                        O2Rum.startView(event.componentId, screenName);
                    ...
                    O2RumReactNativeNavigationTracking.lastView = {
        ...
        O2RumReactNativeNavigationTracking.isTracking = true;
        O2RumReactNativeNavigationTracking.viewNamePredicate = viewNamePredicate;
        O2RumReactNativeNavigationTracking.viewTrackingPredicate = viewTrackingPredicate;
        O2RumReactNativeNavigationTracking.paramsTrackingPredicate = paramsTrackingPredicate;
        this.appStateSubscription = AppState.addEventListener(
            'change',
            O2RumReactNativeNavigationTracking.appStateListener
        );
    }
    ...
    static stopTracking(): void {
        ...
        if (O2RumReactNativeNavigationTracking.eventSubscription) {
            O2RumReactNativeNavigationTracking.eventSubscription.remove();
        }
        ...
        O2RumReactNativeNavigationTracking.lastView = undefined;
        O2RumReactNativeNavigationTracking.isTracking = false;
        O2RumReactNativeNavigationTracking.viewNamePredicate = defaultViewNamePredicate;
        O2RumReactNativeNavigationTracking.viewTrackingPredicate = defaultViewTrackingPredicate;
        O2RumReactNativeNavigationTracking.paramsTrackingPredicate = defaultParamsPredicate;

        // eslint-disable-next-line func-names
        O2RumReactNativeNavigationTracking.viewNamePredicate = function (
            _event: ComponentDidAppearEvent,
            trackedName: string
        ) {
            return trackedName;
    ...
    private static appStateListener: AppStateListener = (
        ...
        const lastView = O2RumReactNativeNavigationTracking.lastView;
        ...
        if (appStateStatus === 'background') {
            ...
            O2Rum.startView(lastView.key, lastView.name);
        }
    };
}
