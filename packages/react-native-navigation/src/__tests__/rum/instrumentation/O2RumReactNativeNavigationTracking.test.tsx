...
afterEach(() => {
    O2RumReactNativeNavigationTracking.stopTracking();
});
...
it('M send a RUM ViewEvent W startTracking() for the first view', async () => {
    // GIVEN
    O2RumReactNativeNavigationTracking.startTracking();
    ...
    // THEN
    await waitFor(() =>
        expect(O2Rum.startView).toHaveBeenCalledWith(expect.any(String), 'Home')
    );
});
...
it('M send a RUM ViewEvent W startTracking() componentDidAppear { custom viewNamePredicate }', async () => {
    ...
    // THEN
    await waitFor(() => expect(O2Rum.startView).toHaveBeenCalledTimes(2));
    expect(O2Rum.startView).toHaveBeenCalledWith(expect.any(String), 'Home');
...
it('M send a RUM ViewEvent W startTracking() componentDidAppear { viewTrackingPredicate always returns true }', async () => {
    ...
    expect(O2Rum.startView).toHaveBeenCalledWith(expect.any(String), 'Home');
    expect(O2Rum.startView).toHaveBeenCalledWith(expect.any(String), 'About');
});
...
describe.each([
    ...
    it('starts last view when app goes into foreground', async () => {
        ...
        // THEN
        expect(O2Rum.stopView).toHaveBeenCalledTimes(1);
        expect(O2Rum.startView).toHaveBeenCalledTimes(3);
        expect(O2Rum.startView).toHaveBeenNthCalledWith(
            3,
            expect.any(String),
            'About'
        );
