...
@objc public class RCTOpenObserveWebViewTracking: NSObject {
    var webView: RCTOpenObserveWebView? = nil
    var allowedHosts: Set<String> = Set()
    var onSdkInitializedListener: OnSdkInitializedListener?
    
    public override init() {
        super.init()
        self.onSdkInitializedListener = { [weak self] (core: OpenObserveCoreProtocol) in
     ...
     */
    @objc public func enable(webView: RCTOpenObserveWebView, allowedHosts: Set<String>) {
        ...
        if CoreRegistry.isRegistered(instanceName: CoreRegistry.defaultInstanceName) {
            ...
            OpenObserveSDKWrapper.shared.addOnSdkInitializedListener(listener: onSdkInitializedListener)
        } else {
    ...
    private func enableWebViewTracking(
        webView: RCTOpenObserveWebView,
        allowedHosts: Set<String>,
        core: OpenObserveCoreProtocol
    ) {
