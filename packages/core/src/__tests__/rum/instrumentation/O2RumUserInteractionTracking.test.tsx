...
it('M intercept and send a RUM event W onPress { Button component }', async () => {
    ...
    // THEN
    expect(O2Rum.addAction).toBeCalledWith(
        'TAP',
        'click_me_button',
        null,
        expect.anything(),
...
describe('startTracking', () => {
    ...
    it('does not crash if jsx-runtime does not contain jsx', () => {
        ...
        expect(O2RumUserInteractionTracking['isTracking']).toBe(false);
        jest.setMock('react/jsx-runtime', {});
        O2RumUserInteractionTracking.startTracking({});
        expect(O2RumUserInteractionTracking['isTracking']).toBe(true);
