...
export class BoundedBuffer extends OpenObserveBuffer {
    ...
    drain = async () => {
        for (
            ...
            try {
                ...
                InternalLog.log(
                    `Error while draining OpenObserve Buffer: ${error}`,
                    SdkVerbosity.WARN
                );
            }
        }
    ...
    private logRandomIdGenerationError = (error: any): void => {
        ...
        console.warn(
            `[OpenObserve] Could not generate enough random numbers for RUM buffer. Please check that Math.random is not overwritten. Math.random returns: ${Math.random()}`
        );
    };
