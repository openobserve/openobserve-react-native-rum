...
import type { O2NativeLogsType } from '../nativeModulesTypes';
...
class O2LogsWrapper implements O2LogsType {
    // eslint-disable-next-line global-require, @typescript-eslint/no-var-requires
    private nativeLogs: O2NativeLogsType = require('../specs/NativeDdLogs')
        .default;
    ...
    private logWithError = async (
        ...
        const updatedContext = {
            ...encodedContext,
            [O2Attributes.errorSourceType]: 'react-native'
        };

        if (fingerprint && fingerprint !== '') {
            updatedContext[O2Attributes.errorFingerprint] = fingerprint;
        }

        const _debugId = debugId;
        if (_debugId) {
            updatedContext[O2Attributes.debugId] = _debugId;
        }
...
export const O2Logs = getGlobalInstance(LOGS_MODULE, () => new O2LogsWrapper());
