 ...
 */
export function __o2ExtractText(node: any, prefer?: any[]): string[] {
    // If caller provided preferred values (title/label/etc.), use those first.
    if (Array.isArray(prefer)) {
        const preferred = prefer
            .flatMap(v => __o2ExtractText(v)) // recurse so expressions/arrays work
            .map(normalize)
            .filter(Boolean);
    ...
    if (React.isValidElement(node)) {
        ...
        for (const propKey of LABEL_PROPS) {
            ...
            if (propValue != null) {
                const got = __o2ExtractText(propValue)
                    .map(normalize)
                    .filter(Boolean);
        ...
        const perChild = children.map(child => __o2ExtractText(child));
