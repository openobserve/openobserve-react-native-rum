...
describe('O2BabelInteractionTracking.wrapRumAction', () => {
    it('should not crash when func is undefined', () => {
        const wrapped = O2BabelInteractionTracking.wrapRumAction(
            undefined as any,
            RumActionType.TAP,
            mockTargetObject
        );
    ...
    it('should forward the first handler argument as actionContext to O2Rum.addAction', () => {
        const mockAddAction = jest.fn().mockResolvedValue(undefined);
        O2BabelInteractionTracking.config = { trackInteractions: true };
        O2BabelInteractionTracking.attachRumInstance({
