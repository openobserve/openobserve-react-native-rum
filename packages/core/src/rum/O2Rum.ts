...
import { O2Attributes } from '../O2Attributes';
...
class O2RumWrapper implements O2RumType {
    // eslint-disable-next-line global-require, @typescript-eslint/no-var-requires
    private nativeRum: O2NativeRumType = require('../specs/NativeDdRum')
        .default;
    ...
    addError = (
        ...
        updatedContext[O2Attributes.errorSourceType] = 'react-native';

        if (debugId) {
            updatedContext[O2Attributes.debugId] = debugId;
        }
    ...
    private getStopActionNativeCallArgs = (
        ...
        if (isOldStopActionAPI(args)) {
            ...
            InternalLog.log(
                'O2Rum.startAction needs to be called before O2Rum.stopAction',
                SdkVerbosity.WARN
            );
        } else {
            InternalLog.log(
                'O2Rum.stopAction was called with wrong arguments',
                SdkVerbosity.WARN
            );
        }
...
export const O2Rum = getGlobalInstance(RUM_MODULE, () => new O2RumWrapper());
