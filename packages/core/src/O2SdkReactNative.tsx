...
import { O2SdkNativeConfiguration } from './config/features/CoreConfigurationNative';
...
import { registerNativeBridge } from './sdk/OpenObserveInternalBridge/O2SdkInternalNativeBridge';
...
export class O2SdkReactNative {
    ...
    static initialize = async (
        ...
        await O2SdkReactNative.initializeNativeSDK(configuration, {
            initializationModeForTelemetry: 'LEGACY'
        });

        O2SdkReactNative.enableFeatures(configuration);
    };
    ...
    private static initializeNativeSDK = async (
        ...
        await NativeDdSdk.initialize(
            O2SdkReactNative.buildConfiguration(configuration, params)
        );
    ...
    static _initializeFromOpenObserveProvider = async (
        ...
        if (configuration instanceof FileBasedConfiguration) {
            return O2SdkReactNative.initializeNativeSDK(configuration, {
        ...
        if (configuration.initializationMode === InitializationMode.ASYNC) {
            const initNative = () =>
                O2SdkReactNative.initializeNativeSDK(configuration, {
        ...
        // TODO: Remove when O2SdkReactNativeConfiguration is deprecated
    ...
    static _enableFeaturesFromOpenObserveProvider = (
        ...
        O2SdkReactNative._enableFeaturesFromOpenObserveProviderAsync(features);
    };

    static _enableFeaturesFromOpenObserveProviderAsync = async (
        features: AutoInstrumentationConfiguration
    ): Promise<void> => {
        O2SdkReactNative.features = features;
        O2SdkReactNative.enableFeatures(
    ...
    static _initializeFromOpenObserveProviderWithConfigurationAsync = async (
        ...
        return O2SdkReactNative.initializeNativeSDK(builtConfiguration, {
    ...
    private static buildConfiguration = (
        ...
        configuration.additionalConfiguration[O2SdkReactNative.DD_SOURCE_KEY] =
            'react-native';
        configuration.additionalConfiguration[
            O2SdkReactNative.DD_SDK_VERSION
        ] = sdkVersion;

        if (configuration.version) {
            configuration.additionalConfiguration[
                O2SdkReactNative.DD_VERSION
        ...
        if (configuration.versionSuffix && !configuration.version) {
            configuration.additionalConfiguration[
                O2SdkReactNative.DD_VERSION_SUFFIX
            ] = `-${configuration.versionSuffix}`;
        }

        if (reactNativeVersion) {
            configuration.additionalConfiguration[
                O2SdkReactNative.DD_REACT_NATIVE_VERSION
            ] = `${reactNativeVersion}`;
        }
        ...
        return new O2SdkNativeConfiguration(
            configuration.additionalConfiguration,
    ...
    private static enableFeatures(
        ...
        if (globalThis.__DD_RN_BABEL_PLUGIN_ENABLED__) {
            O2BabelInteractionTracking.config = {
                trackInteractions,
            ...
            O2BabelInteractionTracking.attachRumInstance(O2Rum);
        }

        if (O2SdkReactNative.wasAutoInstrumented) {
            InternalLog.log(
        ...
        O2SdkReactNative.wasAutoInstrumented = true;
    }
}
