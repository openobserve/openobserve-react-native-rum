...
internal class O2SdkNativeInitializationTest {

    lateinit var testedNativeInitialization: O2SdkNativeInitialization
    ...
    fun `set up`() {
        ...
        testedNativeInitialization = O2SdkNativeInitialization(
            mockContext,
            mockOpenObserve,
            mockDdTelemetry,
            mockJSONFileReader
        ...
        O2SdkSessionStartedListener.resetIsRnSdkInitialized()
    }
    ...
    fun `𝕄 parse configuration file 𝕎 getConfigurationFromJSONFile() {file with all data}`() {
        // Given
        whenever(
            mockJSONFileReader.parseAssetsJSONFile(mockContext, "openobserve-configuration.json")
    ...
    fun `𝕄 mark RN SDK initialized and catch up session 𝕎 initialize() { isCalledFromJs=true, datadog already initialized }`() { // ktlint-disable-line max-line-length
        ...
        whenever(mockOpenObserve.isInitialized()) doReturn true
        ...
        // Then
        assertThat(O2SdkSessionStartedListener.isRnSdkInitializedForTests()).isTrue()
    ...
    fun `𝕄 mark RN SDK initialized without catch up 𝕎 initialize() { isCalledFromJs=true, datadog not initialized }`() { // ktlint-disable-line max-line-length
        // Given
        whenever(mockOpenObserve.isInitialized()) doReturn false
        ...
        // Then
        assertThat(O2SdkSessionStartedListener.isRnSdkInitializedForTests()).isTrue()
        verify(mockOpenObserve, never()).getRumMonitor()
    }
    ...
    fun `𝕄 not mark RN SDK initialized or catch up 𝕎 initialize() { isCalledFromJs=false }`() {
        ...
        // Then
        assertThat(O2SdkSessionStartedListener.isRnSdkInitializedForTests()).isFalse()
        verify(mockOpenObserve, never()).getRumMonitor()
    }

    // endregion

    private fun minimalConfiguration(): O2SdkConfiguration = O2SdkConfiguration(
