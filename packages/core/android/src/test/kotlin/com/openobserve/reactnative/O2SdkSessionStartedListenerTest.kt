...
internal class O2SdkSessionStartedListenerTest {
    ...
    fun `set up`() {
        ...
        O2SdkSessionStartedListener.getInstance().setUiThreadExecutor(TestUiThreadExecutor())
    ...
    fun `𝕄 getInstance implements singleton correctly`() {
        // GIVEN
        val instance1 = O2SdkSessionStartedListener.getInstance()
        // WHEN
        val instance2 = O2SdkSessionStartedListener.getInstance()
        // THEN
    ...
    fun `𝕄 invalidate() invalidates previous instance`() {
        ...
        O2SdkSessionStartedListener.invalidate()
        // THEN
        assertThat(instance1).isNotEqualTo(O2SdkSessionStartedListener.getInstance())
    }
    ...
    fun `𝕄 onSessionStarted internally catches exception if catalyst instance throws`() {
        ...
        val instance = O2SdkSessionStartedListener.getInstance()
    ...
    fun `𝕄 session ID is NOT sent via bridge W { setReactContext fires before onRnSdkInitialized }`() { // ktlint-disable-line max-line-length
        ...
        // THEN — bridge must NOT be called (OpenObserveInternalReactBridge not yet registered)
