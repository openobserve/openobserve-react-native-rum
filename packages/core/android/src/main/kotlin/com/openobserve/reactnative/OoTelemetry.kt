 ...
 * **[INTERNAL USAGE]**
 *
 * Utility class used by React Native modules to forward telemetry events to the OpenObserve SDK.
 *
 * This class is **public only for OpenObserve internal package visibility** and should not be used.
 */
class OoTelemetry {
    ...
    private var telemetryProxy: _InternalProxy._TelemetryProxy? = null
        get() {
            if (field == null && OpenObserve.isInitialized()) {
                field = OpenObserve._internalProxy()._telemetry
            }
    ...
    fun sendTelemetryLog(message: String, attributes: ReadableMap, config: ReadableMap) {
        val core = OpenObserve.getInstance() as FeatureSdkCore?
