...
class O2SdkImplementation(
    ...
    fun initialize(configuration: ReadableMap, promise: Promise) {
        ...
        // On new arch O2Sdk is a lazy TurboModule — it's instantiated on the first JS-side
        ...
        O2SdkSessionStartedListener.getInstance().setReactContext(reactContext)

        val nativeInitialization = O2SdkNativeInitialization(appContext, datadog, ddTelemetry)
    ...
    private fun configureSynthetics() {
        if (O2SdkSynthetics.testId.isNullOrBlank() || O2SdkSynthetics.resultId.isNullOrBlank()) {
            return
        }

        datadog.getRumMonitor()._getInternal()?.setSyntheticsAttribute(
            O2SdkSynthetics.testId,
            O2SdkSynthetics.resultId
        )
    }
    ...
    private fun createFrameRateProvider(
        ddSdkConfiguration: O2SdkConfiguration
    ): FrameRateProvider? {
