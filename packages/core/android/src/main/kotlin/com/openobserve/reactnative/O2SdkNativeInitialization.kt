...
import com.openobserve.android.OpenObserveSite
...
import com.openobserve.android.event.EventMapper
...
import com.openobserve.android.rum._RumInternalProxy
import com.openobserve.android.rum.configuration.VitalsUpdateFrequency
import com.openobserve.android.rum.metric.networksettled.TimeBasedInitialResourceIdentifier
import com.openobserve.android.rum.model.ActionEvent
import com.openobserve.android.rum.model.ResourceEvent
import com.openobserve.android.rum.tracking.ActivityViewTrackingStrategy
import com.openobserve.android.telemetry.model.TelemetryConfigurationEvent
...
/**
 * Initializes the Android OpenObserve SDK.
 */
class O2SdkNativeInitialization internal constructor(
    private val appContext: Context,
    private val datadog: OpenObserveWrapper = OpenObserveSDKWrapper(),
    private val ddTelemetry: O2Telemetry = O2Telemetry(),
    private val jsonFileReader: JSONFileReader = JSONFileReader()
) {
    @Suppress("CyclomaticComplexMethod")
    internal fun initialize(ddSdkConfiguration: O2SdkConfiguration, isCalledFromJs: Boolean = true) {
        ...
        if (isCalledFromJs) {
            O2SdkSessionStartedListener.getInstance().onRnSdkInitialized()
            ...
            if (datadog.isInitialized()) {
                datadog.getRumMonitor().getCurrentSessionId {
                    it?.let { sessionId ->
                        O2SdkSessionStartedListener.getInstance().onSessionStarted(sessionId, false)
                    }
                }
            }
        }
        ...
        if (rumConfiguration != null) {
            Rum.enable(rumConfiguration, OpenObserve.getInstance())
        }

        if (logsConfiguration != null) {
            Logs.enable(logsConfiguration, OpenObserve.getInstance())
        }

        if (traceConfiguration != null) {
            Trace.enable(traceConfiguration, OpenObserve.getInstance())
        }
    ...
    private fun configureRumAndTracesForLogs(configuration: O2SdkConfiguration) {
    ...
    private fun configureSdkVerbosity(configuration: O2SdkConfiguration) {
        val verbosity =
    ...
    private fun getDefaultAppVersion(): String {
        ...
        val packageInfo =
            try {
                ...
                ddTelemetry.telemetryError(e.message ?: O2SdkImplementation.PACKAGE_INFO_NOT_FOUND_ERROR_MESSAGE, e)
                return O2SdkImplementation.DEFAULT_APP_VERSION
            }
        ...
        return packageInfo?.let {
            ...
            ?: O2SdkImplementation.DEFAULT_APP_VERSION
    }

    @Suppress("CyclomaticComplexMethod")
    private fun buildRumConfiguration(configuration: O2SdkConfiguration): RumConfiguration {
        val configBuilder =
        ...
        configBuilder.setResourceEventMapper(
            object : EventMapper<ResourceEvent> {
                override fun map(event: ResourceEvent): ResourceEvent? {
                    if (event.context?.additionalProperties?.containsKey(O2SdkImplementation.DD_DROP_RESOURCE) ==
                        true
                    ) {
                        return null
                    }
        ...
        configBuilder.setActionEventMapper(
            object : EventMapper<ActionEvent> {
                override fun map(event: ActionEvent): ActionEvent? {
                    if (event.context?.additionalProperties?.containsKey(O2SdkImplementation.DD_DROP_ACTION) == true
                    ) {
                        return null
                    }
                    return event
        ...
        configBuilder.setSessionListener(O2SdkSessionStartedListener.getInstance())

        return configBuilder.build()
    }

    private fun buildLogsConfiguration(configuration: O2SdkConfiguration): LogsConfiguration {
    ...
    private fun buildTraceConfiguration(configuration: O2SdkConfiguration): TraceConfiguration {
    ...
    private fun buildSdkConfiguration(configuration: O2SdkConfiguration): Configuration {
        ...
        val versionSuffix = configuration.additionalConfiguration?.get(O2SdkImplementation.DD_VERSION_SUFFIX) as? String
        if (versionSuffix != null && additionalConfig != null) {
            val defaultVersion = getDefaultAppVersion()
            additionalConfig.put(O2SdkImplementation.DD_VERSION, defaultVersion + versionSuffix)
        }
        ...
        if (additionalConfig?.get(O2SdkImplementation.DD_NEEDS_CLEAR_TEXT_HTTP) == true) {
    ...
    private fun buildTrackingConsent(trackingConsent: String?): TrackingConsent {
        return when (trackingConsent?.lowercase(Locale.US)) {
            ...
            "not_granted" -> TrackingConsent.NOT_GRANTED
                ...
                Log.w(
                    O2Sdk::class.java.canonicalName,
    ...
    private fun buildSite(site: String?): OpenObserveSite {
        val siteLower = site?.lowercase(Locale.US)
        return when (siteLower) {
            "us1", "us" -> OpenObserveSite.US1
            "eu1", "eu" -> OpenObserveSite.EU1
            "us3" -> OpenObserveSite.US3
            "us5" -> OpenObserveSite.US5
            "us1_fed", "gov" -> OpenObserveSite.US1_FED
            "us2_fed" -> OpenObserveSite.US2_FED
            "ap1" -> OpenObserveSite.AP1
            "ap2" -> OpenObserveSite.AP2
            else -> OpenObserveSite.US1
        }
    }
    ...
    internal fun getConfigurationFromJSONFile(): O2SdkConfiguration {
        try {
            val jsonString = jsonFileReader.parseAssetsJSONFile(appContext, "openobserve-configuration.json")
    ...
    companion object {
        /**
         * Initializes the OpenObserve React Native SDK from your MainApplication.
         *
         * @param appContext: The application context of your React Native application.
         */
        @JvmStatic
        fun initFromNative(appContext: Context) {
            val nativeInitialization = O2SdkNativeInitialization(appContext.applicationContext)
            try {
                ...
                Log.w(
                    O2SdkNativeInitialization::class.java.canonicalName,
                    "Failed to initialize the OpenObserve SDK: $error"
                )
            }
        }
    }
