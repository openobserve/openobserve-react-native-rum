...
+ (void)initFromNative {
    O2SdkNativeInitialization *nativeInitialization = [[O2SdkNativeInitialization alloc] init];
...
- (O2SdkImplementation*)ddSdkImplementation
{
    if (_ddSdkImplementation == nil) {
        _ddSdkImplementation = [[O2SdkImplementation alloc] initWithBridge:_bridge];
...
- (void)addListener:(NSString *)eventType {
    [O2SdkSessionStartedListener.instance setHasListeners: true];
    [super addListener:eventType];
}

- (void)removeListeners:(double)count {
    [O2SdkSessionStartedListener.instance setHasListeners: false];
...
- (void)registerSessionIdListener {
    __weak O2Sdk* weakSelf = self;
    [O2SdkSessionStartedListener.instance setListenerCallback:^(NSString * _Nonnull sessionId) {
        O2Sdk* strongSelf = weakSelf;
        if (!strongSelf) {
            return;
        }
...
- (void)registerNativeBridge {
    [O2SdkSessionStartedListener.instance setRCTBridge: _bridge];
}
#endif
