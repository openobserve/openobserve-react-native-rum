...
import { O2Rum, SdkVerbosity, InternalLog } from '@openobserve/mobile-react-native';
...
class RumReactNavigationTracking {
    ...
    readonly NAVIGATION_REF_IN_USE_ERROR_MESSAGE =
        'Cannot track new navigation container while another one is still tracked. Please call `O2RumReactNavigationTracking.stopTrackingViews` on the previous container reference.';
        ...
        if (key != null && screenName != null) {
            ...
            if (appStateStatus !== 'background') {
                ...
                if (this.viewTrackingPredicate(route)) {
                    ...
                    if (params) {
                        O2Rum.startView(customKey, screenName, { params });
                    } else {
                        O2Rum.startView(customKey, screenName);
                    }
                }
            }
        }
...
export const O2RumReactNavigationTracking = getGlobalInstance(
