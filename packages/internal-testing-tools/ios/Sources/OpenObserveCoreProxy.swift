...
internal class OpenObserveCoreProxy: OpenObserveCoreProtocol {
    let core: OpenObserveCoreProtocol
    
    private var featureScopeInterceptors: [String: FeatureScopeInterceptor] = [:]

    init(core: OpenObserveCoreProtocol) {
        self.core = core
    }
    ...
    func scope<T>(for featureType: T.Type) -> FeatureScope where T: OpenObserveFeature {
...
private struct FeatureScopeProxy: FeatureScope {
    ...
    func eventWriteContext(bypassConsent: Bool, _ block: @escaping (OpenObserveContext, Writer) -> Void) {
        interceptor.enter()
    ...
    func context(_ block: @escaping (OpenObserveContext) -> Void) {
        interceptor.enter()
...
private final class FeatureScopeInterceptor: @unchecked Sendable {
    struct InterceptingWriter: Writer {
        ...
        func write<T, M>(value: T, metadata: M?, completion: @escaping OpenObserveInternal.CompletionHandler) where T : Encodable, M : Encodable {
            group.enter()
            defer { group.leave() }
...
extension OpenObserveCoreProxy {
