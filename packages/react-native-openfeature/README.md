...
**Note**: This package is an integration for the [OpenFeature React SDK][1]. Install and set up the core [`@openobserve/mobile-react-native`][3] SDK to start using Datadog Feature Flags.
...
```tsx
import { CoreConfiguration, DatadogProvider, OoFlags } from '@openobserve/mobile-react-native';
...
> **Note**: Sending flag evaluation data to Datadog is automatically enabled when using the Feature Flags SDK. Provide `rumIntegrationEnabled` and `trackExposures` parameters to the `OoFlags.enable()` call to configure.
